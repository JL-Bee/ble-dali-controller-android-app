...
fun BluetoothGattService.getZsc010Characteristic(
  service: String,
  characteristic: Long,
  macAddress: Long
...
fun BluetoothGatt.zsc010ServiceMatchingMask(serviceMask: String): BluetoothGattService? {
  val serviceToMatchMaskUUID = UUID.fromString(serviceMask)
  val serviceMaskUUID = UUID.fromString(ZSC010_BLUETOOTH_SERVICE_MASK)
...
val BluetoothGattCharacteristic.serviceMask: UUID
  get() = this.service.uuid and UUID.fromString(ZSC010_BLUETOOTH_SERVICE_MASK)
